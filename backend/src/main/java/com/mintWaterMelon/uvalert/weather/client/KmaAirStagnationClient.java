...
@Component
public class KmaAirStagnationClient {

    private final RestClient restClient;
    private final String serviceKey;
    private final String airStagnationPath;

    public KmaAirStagnationClient(
            RestClient.Builder restClientBuilder,
            @Value("${kma.living-weather.base-url}") String baseUrl,
            @Value("${kma.service-key}") String serviceKey,
            @Value("${kma.living-weather.air-stagnation-path}") String airStagnationPath
    ) {
        this.restClient = restClientBuilder
                .baseUrl(baseUrl)
                .build();
        this.serviceKey = serviceKey;
        this.airStagnationPath = airStagnationPath;
    }

    public WeatherHourlyIndexResponse getAirStagnationIndex(String areaNo, String time) {
        JsonNode response = restClient.get()
                .uri(uriBuilder -> uriBuilder
                        .path(airStagnationPath)
        ...
        validateResponse(response, "대기정체지수 API");
    ...
    private JsonNode extractFirstItem(JsonNode response) {
        ...
        if (item == null || item.isMissingNode() || item.isEmpty()) {
            throw new IllegalStateException("대기정체지수 API 응답에 item 데이터가 없습니다.");
        }
